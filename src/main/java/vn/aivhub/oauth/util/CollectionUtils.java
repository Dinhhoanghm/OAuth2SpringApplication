...
import vn.aivhub.oauth.config.jackson.json.JsonArray;
...
public class CollectionUtils {
  ...
  public static <K, V> Map<K, V> collectToMap(Collection<V> list, Function<V, K> keyFunction) {
    return list.stream()
      .filter(v -> keyFunction.apply(v) != null)
      .collect(toMap(keyFunction, Function.identity(), (v1, v2) -> v1));
  }
  ...
  public static <T, K, V> Map<K, V> collectToMap(Collection<T> list, Function<T, K> keyFunction, Function<T, V> valueFunction) {
    return list.stream()
      .filter(t -> keyFunction.apply(t) != null && valueFunction.apply(t) != null)
  ...
  public static List<Integer> convertToListInt(String input) {
    if (input == null || input.contains("[]")) return Collections.emptyList();
    try {
      return new JsonArray(input)
        .stream()
        .filter(o -> NumberUtils.isDigits(o.toString()))
        .map(o -> Integer.parseInt(o.toString()))
        .toList();
    } catch (Exception e) {
